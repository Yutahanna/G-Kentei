...
## 21. validate:questions警告172件の分類

全250問に対する `npm run validate:questions` の警告172件を、公開判断の観点から
「意図的で問題なし」「将来改善候補」「本番公開前に再確認必須」の3分類に整理した。
分類の詳細・根拠は `docs/warning-classification.md` を参照。要約は以下のとおり。

| カテゴリ | 件数 | 分類 |
|---|---|---|
| 論点重複の疑い | 19 | 意図的で問題なし |
| 誤答選択肢の近接概念不足 | 11 | 将来改善候補 |
| 正答文の長さ偏り | 135 | 将来改善候補（件数が多く優先度は高い） |
| 誤答選択肢の極端な断定表現（第1・2章のみ） | 7 | 本番公開前に再確認必須 |
