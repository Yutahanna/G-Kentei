...
### 23.4 否定形設問の目標比率の確定（ユーザー指摘対応・追加）

23.3で「未着手」としていた否定形設問の目標比率について、ユーザーから追加の指摘
「不適切な回答を選ぶ問題は本番と同じ比率にしてください」を受け、以下のとおり
方針を確定した。

- **確認基準日**: 2026-08-29。
- **根拠**: JDLA本番試験における否定形設問（「不適切なものを選べ」等）の出題比率は、
  本アプリの教材（`materials/`）にも、Web検索で確認できた範囲の公開情報にも
  明記がなかった（G検定は145問・100分の試験であることは確認できたが、設問形式の
  内訳を示す一次情報は見つからなかった）。ユーザーに根拠を確認したところ、
  「購入したテキスト（本アプリの教材とは別の市販G検定対策教材）では、収録問題の
  約半数が「不適切なものを選ぶ」形式である」との回答を得た。この情報はユーザー
  提供の一次情報であり、JDLA公式の統計として検証されたものではない。試験形式は
  今後変更されうるため、この基準日以降にJDLA公式情報等で異なる比率が確認された
  場合は、本方針を再確認すること。
- **確定した目標比率**: 各章25問（基礎10・標準10・応用5）に対し、おおむね半数
  （基礎5問・標準5問・応用2問、計12問＝48%）を否定形設問とする。25問を均等に
  半分にはできないため、応用問題側で切り捨て、全体比率が50%をわずかに下回る
  よう保守的に倒した。
- **適用方法**: 第3章の追加試作として、既存の肯定形設問のうち8問
  （`ch03-basic-001/004/006/009`、`ch03-standard-003/005/007/009`、
  `ch03-advanced-001/004`）を、教材本文に直接矛盾する1つの誤った記述と、
  教材の記述に合致する3つの真の記述からなる否定形設問に全面的に書き換えた
  （既存の`ch03-basic-003`・`ch03-standard-001`と合わせて章内12問＝48%）。
  出題ID・章・節・難易度・`contentVersion`は変更せず、設問文・選択肢・正解・
  解説のみを教材本文（`materials/chapters/06_第3章.md`）から新たに根拠づけて
  書き直した。`npm run validate:questions`（エラー0件・警告26件、うち第3章
  由来の新規警告なし）、`typecheck`、`test`（81件成功）、および実アプリでの
  Playwright動作確認（否定形設問の表示・採点・解説、コンソールエラー0件）で
  品質を確認した。
- **展開方針**: 本比率（各章48%目安）を残り9章（第1・2・4〜10章）に同様の
  手法で展開する。
