...
作成日: 2026-08-02／改訂: 2026-08-03（今回のWebアプリ化対応との関係を明記）

## 進捗段階: 保留（今回の対応範囲外）

主な配布方針は `docs/web-hosting-deployment.md` に記載の非公開Webアプリ（Cloudflare Pages +
Cloudflare Access）へ切り替えたため、本ドキュメントが示すWindowsデスクトップ版は
**今回のWebアプリ化対応の対象外**であり、修正は行っていない。コード・成果物・原因記録は
将来の再開に備えてそのまま残す。

## ステータス（重要）: 0.1.0は現時点でWindows実機では起動できない

実機検証の結果、`v0.1.0`のビルド成果物（インストーラー版・ポータブル版とも）は以下の
不具合により**Windows実機で起動できないことを確認した**。

- **SmartScreen警告**: 未署名アプリのため表示される（想定内、`詳細情報`→`実行`で回避可能）。
- **WebView2Loader.dll 不足**: 実行時に`WebView2Loader.dll`が見つからずアプリが起動しない。
  本セッションはLinuxサンドボックスからのクロスコンパイルであり、Windows実機でのビルドで
  通常自動的に配置される`WebView2Loader.dll`（Tauri/wryがビルド時にWindows環境の
  NuGetパッケージ経由で取得・配置するランタイムDLL）が、クロスコンパイル環境には存在せず
  同梱されなかったことが原因と考えられる。本ドキュメント下部の「クロスコンパイルは実験的機能」
  という既知の制約が、まさにこの形で顕在化した。

このため、**今回の依頼では、Windowsデスクトップ版の修正は優先せず、既存のReact/Vite/PWA
アプリを非公開のWebアプリとして提供する方針に切り替えた**（詳細は`docs/web-hosting-deployment.md`）。
デスクトップ版のコード・ドキュメントは削除せず、将来Windows実機上でビルドし直す等の対応を
検討する際の参考として残す。仮に再開する場合、最低限次のいずれかの対応が必要になる見込み。

- Windows実機（またはGitHub ActionsのWindowsランナー）でビルドし直す（`WebView2Loader.dll`が
  正しく同梱される、公式にサポートされたビルド経路）。
- クロスコンパイル環境のまま進める場合は、Microsoft Edge WebView2 SDKの
  `Microsoft.Web.WebView2`パッケージから`WebView2Loader.dll`を別途取得し、
  出力先に手動配置する方法を検証する（未検証・保証なし）。
