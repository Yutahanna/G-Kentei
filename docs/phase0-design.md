...
作成日: 2026-08-01（初版）／改訂: 2026-08-01（承認内容反映版・第1章25問承認版・第2章以降の運用方針確定版）
対象: G検定学習・演習アプリ スクラッチ開発
ステータス: 技術方針・データ方針、および第1章25問（`reviewStatus: approved`）は承認済み。第2章25問は作成・自動検査済みでユーザー確認待ち（本書15節）。第3章以降は本書16節の運用方針（2章ずつのバッチ、重点レビュー方式）で進める。
...
- 基礎2問・標準3問・応用3問の代表サンプル（節の並び順で先頭から選ぶ）
- 自動検査で警告が出た全問題（類似問題、論点重複、近接概念チェック）
- `crossChapterTags`を持つ全問題（教材横断の接続を扱うため、正確性の確認価値が高い）

通常は合計8〜12問程度が重点レビュー対象となる。第1章・第2章は基準点として全問詳細表示（`--focused`なし）を維持し、第3章以降はこの重点レビュー方式を既定とする。
